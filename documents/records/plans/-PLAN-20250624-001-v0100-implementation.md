...
- package.json更新（依存関係追加）
  - postinstallスクリプトの設定（[INST001](../../visions/specifications/installation/inst001-post-install-setup.md)参照）
- ディレクトリ構造作成
- bin/cctop実行ファイル作成
- 設定ファイルの初期化（[CONFIG001](../../visions/specifications/config/config001-management.md)参照）
...
const schema = {
  ...
  events: `
    CREATE TABLE events (
      ...
      is_directory INTEGER DEFAULT 0,  -- 新規追加
...
**自動監視対象追加機能**:
cctop起動時、現在のディレクトリ（または--watchで指定されたディレクトリ）が`monitoring.watchPaths`に含まれていない場合：

1. **ユーザー確認プロンプト表示**:
   ```
   📁 /path/to/current をモニタ対象に追加しますか？ (y/n): 
   ```

2. **応答に基づく処理**:
   - `y/yes`: config.jsonのwatchPathsに絶対パスで追加して保存、監視開始
   - `n/no`: 現在の設定で監視開始（追加なし）

3. **テスト環境での動作**:
   - `NODE_ENV=test`時は自動的にy応答として処理
   - 非対話環境での自動化に配慮

4. **実装における注意点**:
   - 初期設定では`watchPaths`は空リスト
   - 全てのパスは絶対パスで統一管理
...
async function collectMetadata(targetPath, stats) {
  ...
  return {
    ...
    line_count: isDirectory ? 0 : await countLines(targetPath),
