...
1. **CLI引数** - 一時的なオーバーライド
2. **config.json** - 必須設定ファイル（FUNC-100によるローカル/.cctop/またはグローバル~/.cctop/）
...
### **FUNC-100との役割分担**
- **FUNC-100**: ローカル（.cctop/）とグローバル（~/.cctop/）の優先順位管理
- **FUNC-101**: デフォルト設定値とconfig.jsonスキーマの定義・階層的マージ処理

### **他機能との統合ポイント**
- **FUNC-000**: データベースパス設定の管理
- **FUNC-103**: postinstall時のconfig.json初期作成との連携
...
### **設定管理の設計原則**
- **設定の一元化**: config.jsonが唯一の設定ソース（FUNC-100による.cctop/または~/.cctop/）
