...
## 権限範囲
- ✅ `tests/`, `cypress/` - テスト関連ディレクトリ全権
- ✅ `dist/`, `build/` - ビルド成果物管理
- ✅ デプロイ設定・CI/CD設定
- ✅ 本番環境へのデプロイ実行
- ✅ `documents/records/` - 記録系共同編集
- ✅ `documents/visions/specifications/`, `documents/visions/blueprints/` - Architect/Clerk/Builder共同編集
- ✅ `passage/handoffs/` - Builder間受け渡しシステム

### ソースコードディレクトリ制限
- 🔍 `src/` - **読み取り専用**（検証・品質確認のみ）
- ❌ `src/` - **編集禁止**（ユーザー明示許可なしでの編集は絶対禁止）
...
- ❌ **ソースコード編集禁止**: `src/`ディレクトリの編集（ユーザー明示許可なし）
- ❌ 仕様変更の独断決定
...
2. **設計書や企画書・計画書の確認**: 関連する書類を必ず読み返す
...
- 設定パラメータの型・範囲・必須項目
**実践**:
- JSON Schema、Yup、Joi等のバリデーションライブラリ活用
- 不正な設定値での動作確認
...
テストファイルは基本的に `test/` ディレクトリに作成すること：
- ✅ 優先: `test/` 内に永続的なテストファイルを配置
- ⚠️ 一時的な検証のみ: `/tmp/cctop-validate-*.js` 等

**理由**: テストは資産として管理・一時ファイルは最小限に