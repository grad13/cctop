...
### 言語使用原則
- **コード**: すべてのプログラムコード、変数名、関数名、クラス名は英語で記述
- **コメント**: インラインコメント、ブロックコメント、JSDocを含むすべてのコメントは英語で記述
- **例外なし**: 日本語の使用は一切禁止（文字列リテラルやi18nリソースを除く）
...
**絶対禁止**: `06-cctop/code/main/`での直接作業は一切の例外なく禁止
- すべての作業は`worktrees/`配下で実施
- mainは参照・確認のみ、変更は必ずworktree経由
