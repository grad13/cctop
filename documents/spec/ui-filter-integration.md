...
| Source | Runtime |
|--------|---------|
| modules/view/src/ui/ | Node.js |

| Field | Value |
|-------|-------|
| Related | view-display-integration.md, event-type-filtering.md, filter-state-management.md |
| Test Type | Unit |

## 1. Overview

3つのフィルター機能（Display Mode、Event Filter、Keyword Filter）の統合仕様。集合論的アプローチによる一貫した動作と、**Unique処理 = 各ファイルの最新eventを表示する**定義により処理順序問題を根本解決する。
