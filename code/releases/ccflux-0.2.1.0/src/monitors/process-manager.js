...
class ProcessManager {
   ...
   */
  async startMonitor(monitorScriptPath, options = {}) {
    try {
   ...
   */
  async savePidFile(pid, scriptPath, options = {}) {
    const pidInfo = {
      pid: pid,
   ...
   * Wait for process to exit
   */
  async waitForProcessExit(pid, timeout = 5000) {
   ...
   */
  async killOrphanedMonitors(scriptPath) {
    try {
      const { execSync } = require('child_process');
