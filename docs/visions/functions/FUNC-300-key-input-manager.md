...
**作成者**: Architect Agent  
**対象バージョン**: v0.3.5.0  
**関連仕様**: FUNC-202, FUNC-203, FUNC-208, FUNC-400, FUNC-401  
...
- **filtering → waiting**: Escキー入力時（編集破棄、フィルタ設定前の状態を保持）
- **filtering → waiting**: Enterキー入力時（編集保持、フィルタ設定を上書き）
- **searching → waiting**: Escキー入力時（編集破棄、検索設定前の状態を保持）
- **searching → waiting**: Enterキー入力時（編集保持、検索設定を上書き）
- **searching**: Shift+Enterキー入力時（DB検索実行、状態遷移なし）
...
KeyInputManager.initializeStateMaps() {
  ...
  // Filtering/Searching モードのキー登録
  this.registerToState('filtering', 'Escape', FUNC202.discardEditsAndRestorePrevious);
  this.registerToState('filtering', 'Enter', FUNC202.applyEditsAndUpdateState);
  this.registerToState('searching', 'Escape', FUNC202.discardSearchAndRestorePrevious);
  this.registerToState('searching', 'Enter', FUNC202.applySearchAndUpdateState);
  this.registerToState('searching', 'Shift+Enter', FUNC202.applySearch);
}
```
