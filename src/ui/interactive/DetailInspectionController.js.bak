...
class DetailInspectionController {
  constructor(aggregateDisplay, historyDisplay, keyInputManager) {
    ...
    this.renderController = null; // Will be set from InteractiveFeatures
  ...
  async activateDetailMode(selectedFile) {
    ...
    if (typeof selectedFile === 'object') {
      if (selectedFile.name && selectedFile.name.trim()) {
        filePath = selectedFile.path ? `${selectedFile.path}/${selectedFile.name}` : selectedFile.name;
      } else {
  ...
  async refresh() {
    if (this.active && this.selectedFile) {
      try {
        ...
        await this.aggregateDisplay.initialize(this.selectedFile);
        await this.historyDisplay.initialize(this.selectedFile);
