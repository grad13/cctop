...
class ProgressiveLoader extends EventEmitter {
  constructor(databaseManager, displayManager, statusDisplay) {
    ...
    this.loadedCount = 0;
    this.batchSize = 100; // Load events in batches
    this.loadDelay = 10; // Delay between batches (ms)
  ...
  async loadRecentEventsFirst(limit = 50) {
    try {
      ...
      const recentEvents = await this.db.getRecentEvents(limit);
      ...
      if (recentEvents.length > 0) {
        ...
        this.lastLoadedEventId = Math.max(...recentEvents.map(e => e.id || 0));
