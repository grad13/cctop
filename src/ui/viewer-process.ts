...
import ConfigManager = require('../config/config-manager');
...
class ViewerProcess {
  ...
  constructor(config: ViewerProcessConfig = {}) {
    this.config = config;
  }
   ...
   */
  private async ensureMonitorRunning(): Promise<void> {
    try {
   ...
   */
  private startMonitorStatusCheck(): void {
   ...
   * Get monitor logs
   */
  async getMonitorLogs(lines: number = 50): Promise<string[]> {
    try {
